...
def normalizar_intencion(texto: str) -> dict:
    ...
    """
    if '_' not in texto:
        raise ValueError(f"El título '{texto}' no contiene guión bajo para separar subtítulo")
...
def es_titulo_valido(line_text: str, spans: list) -> bool:
    ...
    formato_valido = re.match(r"^[A-Z][a-z]+(?:[A-Z][a-z]+)*_[a-z0-9_]+$", line_text.strip()) is not None
    
    # Verificar estilo de fuente
    estilo_valido = any(
        span.get("flags", 0) == 16 and  # Negrita
        "Calibri-Bold" in span.get("font", "")  # Fuente Calibri-Bold
        for span in spans
    )
